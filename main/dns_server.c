...
static int parse_dns_request(char *req, size_t req_len, char *dns_reply, size_t dns_reply_max_len)
{
    ...
    ESP_LOGD(DNS_TAG, "DNS query with header id: 0x%X, flags: 0x%X, qd_count: %d",
    ...
    for (int i = 0; i < qd_count; i++) {
        ...
        if (name_end_ptr == NULL) {
            ESP_LOGE(DNS_TAG, "Failed to parse DNS question: %s", cur_qd_ptr);
            return -1;
        }
        ...
        ESP_LOGD(DNS_TAG, "Received type: %d | Class: %d | Question for: %s", qd_type, qd_class, name);
        ...
        if (qd_type == QD_TYPE_A) {
            ...
            ESP_LOGD(DNS_TAG, "Answer with PTR offset: 0x%X and IP 0x%X", ntohs(answer->ptr_offset), ip_info.ip.addr);
...
void dns_server_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        ESP_LOGI(DNS_TAG, "Socket created");

        int err = bind(sock, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
        if (err < 0) {
            ESP_LOGE(DNS_TAG, "Socket unable to bind: errno %d", errno);
        }
        ESP_LOGI(DNS_TAG, "Socket bound, port %d", DNS_PORT);

        while (1) {
            ESP_LOGI(DNS_TAG, "Waiting for data");
            ...
            // Data received
                ...
                ESP_LOGI(DNS_TAG, "Received %d bytes from %s | DNS reply with len: %d", len, addr_str, reply_len);
                if (reply_len <= 0) {
                    ESP_LOGE(DNS_TAG, "Failed to prepare a DNS reply");
                } else {
                    int err = sendto(sock, reply, reply_len, 0, (struct sockaddr *)&source_addr, sizeof(source_addr));
                    if (err < 0) {
                        ESP_LOGE(DNS_TAG, "Error occurred during sending: errno %d", errno);
                        break;
                    }
                }
            }
